...
- SoftHSM 或真实 HSM 的 PKCS#11 Live Test；
- DeepSeek 官方 SDK 的真实凭据单节点与多节点 Live Test；
- HSM 厂商对 Ed25519 Object 属性、并发会话和密钥轮换的兼容性；
- 组织级审计、法律和数据保留策略确认。

这些项目不是本地单元测试能够替代的开发任务，必须在目标部署环境中形成验收记录。v1.0 的结论应表达为“代码完成、自动化验收通过、外部环境验收待执行”，不能表达为所有生产条件已经验证。
...
目标是证明编译产物和运行证据可验证。代码与自动化测试已经完成；退出门槛还包括目标环境的 DeepSeek 与 PKCS#11 Live Test 报告。
...
v1.1 的退出门槛是预设组件测试、CLI 预检测试、Dev/Regulated DeepSeek 回归、全量测试和文档验收全部通过。
