...
v1.1 将运行时能力收敛为 `dev`、`team` 和 `regulated` 三档。预设只选择事件存储、签名、根信任、租约和保留期，不改变 Graph 或 Agent 语义。`team` 与 `regulated` 是安全下限，不能通过附加参数降级到 JSONL；专家参数只能增强预设或调整不破坏安全下限的运维值。DeepSeek `run` 默认使用 `regulated`，显式 `--profile dev` 才允许无运行签名的本地执行。
